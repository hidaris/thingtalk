...
def http_request(method, path, data=None):
    ...
    if data is None:
        response = requests.request(method, url, headers=headers, proxies=proxies)
    else:
        headers["Content-Type"] = "application/json"
        response = requests.request(
            method, url, headers=headers, json=json.dumps(data), proxies=proxies
        )
...
def run_client():
    ...
    ws.send(json.dumps({"messageType": "setProperty", "data": {"brightness": 10,}}))
    ...
    ws.send(
        json.dumps({"messageType": "addEventSubscription", "data": {"overheated": {},}})
    )
    ws.send(
        json.dumps(
            {
